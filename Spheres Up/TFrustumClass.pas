...
var
  Magnitude : Single;
begin
  // Calculate the enlargement of the "normal" of the area
  Magnitude := Sqrt(Sqr(pFrustum.Frustum[pPlane][A])+
               Sqr(pFrustum.Frustum[pPlane][B])+Sqr(pFrustum.Frustum[pPlane][C]));
...
begin
  // Save the matrices in arrays using OpenGL
  glGetFloatv(GL_PROJECTION_MATRIX, @ProjM);
  glGetFloatv(GL_MODELVIEW_MATRIX, @ModM);

  // Multiply the projection and model view matrix to obtain the "clipping planes".
  ...
  // Extract the pages of the frustum from the clipping matrix calculated above.
